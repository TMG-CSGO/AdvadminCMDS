...
public Action:Command_Say(client, const String:command[], argc)
{
	...
	if (IsSettingPassword[client] || IsSettingIdent[client] || IsAddingGroup[client])
	{
		...
		if (StrContains(buffer, "cancel", false) != -1)
		{
			IsSettingIdent[client] = false;
			IsSettingPassword[client] = false;
			PrintToChat(client, "\x04[\x01TMG-AdminManager\x04] %t", "Canceled");
	...
	if (IsSettingPassword[client])
	{
		...
		{
			PrintToChat(client, "\x04[\x01TMG-AdminManager\x04] %t", "Fail");
			ShowAdminEditor(client);
			return Plugin_Handled;
		}
		
		KvSetString(h_AdminList, "password", buffer);
		PrintToChat(client, "\x04[\x01TMG-AdminManager\x04] %t", "SetPassword");
	...
	else if (IsSettingIdent[client])
	{
		...
		if (!strcmp(authbuf, "steam", false))
		{
			...
			{
				PrintToChat(client, "\x04[\x01TMG-AdminManager\x04] %t", "InvalidIdent", authbuf);
				ShowAdminEditor(client);
		...
		PrintToChat(client, "\x04[\x01TMG-AdminManager\x04] %t", "SetIdent");
	...
	else if (IsAddingGroup[client])
	{
		...
		PrintToChat(client, "\x04[\x01TMG-AdminManager\x04] %t", "GroupAdded");
